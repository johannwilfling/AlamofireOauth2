...
public func retrieveAuthTokenWithClientCredentials(settings: Oauth2Settings?, performWithToken: (token: String) -> (), errorHandler: () -> () ) {
    ...
    let client = OAuth2Client(outh2Settings: settings!)
    client.retrieveAuthTokenWithClientCredentials({ (authToken) -> Void in
        if let optionnalAuthToken = authToken {
            if authToken != "" {
                print("Received access token " + optionnalAuthToken)
                performWithToken(token: optionnalAuthToken)
